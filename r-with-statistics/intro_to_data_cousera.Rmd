...
nycflights_mod <- nycflights %>%
  mutate(avg_speed = distance / air_time) %>% 
  arrange(desc(avg_speed))
nycflights_mod %>% select(avg_speed, tailnum)
```
...
ggplot(data = nycflights_mod, aes(x = avg_speed, y = distance)) +
  geom_point()
```
...
nycflights_mod2 <- nycflights_mod %>%
 mutate(arr_type = ifelse(arr_delay <= 0,"on time", "delayed"))

nycflights_mod3 <- nycflights_mod2 %>%
  group_by(dep_type) %>%
